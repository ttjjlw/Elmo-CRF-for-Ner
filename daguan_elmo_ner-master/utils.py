...
def save_model(sess, model, path, logger,steps):
    checkpoint_path = os.path.join(path, "ner.ckpt")
    model.saver.save(sess, checkpoint_path,global_step=steps)
    logger.info("model saved in {}".format(checkpoint_path))
...
def create_model(session, Model_class, path, elmo_model, config, logger):
    ...
    #ckpt有两个属性：ckpt.model_checkpoint_path 和ckpt.all_model_checkpoint_paths
